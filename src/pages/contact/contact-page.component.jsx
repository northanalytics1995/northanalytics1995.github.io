...
class ContactPage extends React.Component {
    constructor(props) {
        ...
        this.service_id = "service_5g4j04l";
        this.template_id = "template_2b61izp";
        this.user_id = "user_E1Chew1zIWrHcdWZzggyc";
    ...
    handleFormSubmit = (e) => {
        ...
        } else if (!this.state.messageError&&!this.state.messageSent&&!this.state.messageSending){ // No error and not yet sent
            this.recaptchaRef.props.grecaptcha.execute().catch(error => {this.setState({messageError: true}); console.error("ReCAPTCHA Not available")});
    ...
    handleRecaptcha = async () => {
        const token = this.recaptchaRef.props.grecaptcha.getResponse();
        this.sendRestEmail(token);
    }
    sendEmailForm = (e) => {
        emailjs.sendForm('service_5g4j04l', 'template_2b61izp', e.target, 'user_E1Chew1zIWrHcdWZzggyc')
            .then((result) => {
    ...
    sendRestEmail = async (token) => {
        if (!this.state.messageSent&&!this.state.messageError&&!this.messageSending){
            this.setState({
                messageSending:true,
            });
            const data = {
    ...
    sendEmailDummy = (e) => {
        setTimeout(() => this.setState({messageSent:true, messageSending:false}), 1000);
    ...
    render() {
        return (
            <div className="contact-page-component">
                <TitleUnit
                    title="Get in touch"
                    titleParagraph="Coffee lover? Let’s get to know each other over a lovely cup of coffee, we don't need a reason. I'm sure we'll have a great time together."
                    imageSrc={image}
                    button={<div 
                        ...
                        >Sure <i className="fas fa-caret-down ml-1"></i></div>}
                />
                <div className="background-light background"  ref={(div) => {this.scrollToThisRef = div}} style={{backgroundImage:`url(${bg})`}}>
                        ...
                        <div className="wrapper py-3">
                            ...
                            >
                                ...
                                <div className="form-group">
                                    <label className="color-grey text-small mb-1" htmlFor="input-name">Name</label>
                                    <input 
                                        required
                                        name="sender-name"
                                        type="name" 
                                    ...
                                    ></input>
                                    <label className="color-grey text-small mt-3 mb-1" htmlFor="input-email">Email</label>
                                    <input 
                                        required
                                        name="sender-email"
                                        type="email" 
                                        ...
                                        ></input>
                                    <label className="color-grey text-small mt-3 mb-1" htmlFor="input-message">Message</label>
                                    <textarea 
                                        required
                                        className="form-control text-small" 
                                        id="input-message" 
                                        ...
                                        name="message"
                                        // defaultValue="a"
                                        onChange={e => this.setState({senderMessage: e.target.value})}
                                        ></textarea>
                                </div>
                                <ReCAPTCHA
                                    ref={(div) => this.recaptchaRef = div}
                                    size="invisible"
                                    sitekey={this.recaptchaKey}
                                    // onChange={(value) => this.setState({recaptchaNotPassed:false})}
                                    onChange={this.handleRecaptcha}
                                    badge="inline"
                                />
                                <div className="mt-3"> 
                                    <button 
                                        className={`theme-button color-theme-dark background-light text-small mr-3 ${this.state.messageError||this.state.messageSent?"global-button-disabled":""}`} 
                                        disabled={this.state.messageError||this.state.messageSent}
                                    >Submit</button> 
                                    
                                    {this.state.messageSent?
                                    <div className="d-inline-block text-small color-green"> Hooray, message sent <i className="fas fa-check"></i></div>
                                    :null
                                    }
                                    {this.state.messageError?
                                    <div className="d-inline-block text-small color-theme-contrast"> Uhmm, something went wrong. <i className="fas fa-exclamation"></i></div>
                                    :null
                                    }

                                    {this.state.messageSending?
                                    <div className="spinner-border spinner-border-sm color-theme-dark" role="status">
